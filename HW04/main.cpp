...
void scroll_callback(GLFWwindow* window, double x, double y)
{
  ...
  if (g_camera.fovy() > 45.0f)
    g_camera.set_fovy(45.0f);
  // if (g_camera.fovy() >= 1.0f && g_camera.fovy() <= 45.0f)
  //   g_camera.set_fovy(g_camera.fovy() - y);
  // if (g_camera.fovy() <= 1.0f)
  //   g_camera.set_fovy(1.0f);
  // if (g_camera.fovy() >= 45.0f)
  //   g_camera.set_fovy(45.0f);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_D && action == GLFW_PRESS)
    g_camera.move_right(cameraSpeed);
  if (key == GLFW_KEY_A && action == GLFW_PRESS)
    g_camera.move_left(cameraSpeed);
  if (key == GLFW_KEY_S && action == GLFW_PRESS)
    // g_camera.move_forward(cameraSpeed);
    g_camera.move_backward(cameraSpeed);
...
void set_transform() 
{
  ...
  if (g_is_perspective) {
    mat_proj = glm::perspective(glm::radians(g_camera.fovy()), 800.0f/800.0f, 0.1f, 100.0f);    
    // mat_model = glm::scale(glm::mat4(1.0f), glm::vec3(0.5f, 0.5f, 0.5f));
  }
  else {
