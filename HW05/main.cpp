...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  ...
  {
    // objects[obj_select_idx].print_info(obj_select_idx);
    std::ofstream fout("current_scene_info.txt");
    fout << object_names.size() << std::endl;
    for (int i = 0; i < object_names.size(); i++)
    {
      fout << object_names[i] << std::endl;
      fout << objects[i].scale().x << " " << objects[i].scale().y << " " << objects[i].scale().z << std::endl;
      fout << objects[i].translate().x << " " << objects[i].translate().y << " " << objects[i].translate().z << std::endl;
    }
...
void render_object()
{
  ...
  if(g_is_perspective)
  {
    mat_proj = glm::perspective(cameras[cam_select_idx].fovy(), 1000.0f/1000.0f, 0.1f, 100.0f);
  }
  else
  {
